...
class Student:
	def __init__(self,root):
		...
		roll_lbl=Label(entry_frame,text="student id",font=("",13))
		...
		roll_entry=Entry(entry_frame,bd=3,relief='ridge',font=("",12),textvariable=self.student_id)
		...
		def add_data():
			if self.student_id.get()=="" or self.name.get()==""  or self.father_name.get()=="" or self.contact.get()=="" or self.add_txt.get('1.0',END)=="" or len(self.contact.get())!=10:
				messagebox.showerror('Error','Required all fields and correct field')
			else:
				con=pymysql.connect(host='localhost',user='root',password='admin',database='student_database')
				cur=con.cursor()
				cur.execute('insert into students values(%s,%s,%s,%s,%s,%s,%s,%s,%s)',(self.student_id.get(),self.name.get(),self.father_name.get(),self.gen.get(),self.category.get(),self.branch.get(),self.year.get(),self.contact.get(),self.add_txt.get('1.0',END)))
				con.commit()
				con.close()	
				fetch_data()
				clear_data()
		...
		def update_data():
			if self.student_id.get()=="" or self.name.get()==""  or self.father_name.get()=="" or self.contact.get()=="" or self.add_txt.get('1.0',END)=="" or len(self.contact.get())!=10:
				...
				con=pymysql.connect(host='localhost',user='root',password='admin',database='student_database')
				cur=con.cursor()
				cur.execute('update students set Name=%s , Father_Name=%s , Gender=%s , Category=%s , Branch=%s , Year=%s , Contact_no=%s , Address=%s where student_id=%s ',(self.name.get(),self.father_name.get(),self.gen.get(),self.category.get(),self.branch.get(),self.year.get(),self.contact.get(),self.add_txt.get('1.0',END),self.student_id.get()))
				con.commit()
				con.close()
				fetch_data()
				clear_data()
		...
		search_combo['values']=('student_id','Contact_no')
		...
		table=ttk.Treeview(view_frame,columns=('student_id','Name','Father_Name','Gender','Category','Branch','Year','Contact_no.','Address'),xscrollcommand=x_scroll.set,yscrollcommand=y_scroll.set)
		...
		table.heading("student_id",text="student_id")
		...
		table.column("student_id",width=100)
